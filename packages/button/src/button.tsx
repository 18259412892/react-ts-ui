...
type INativeButtonProps = IButtonProps & React.ButtonHTMLAttributes<HTMLElement>;
type IAnchorButtonProps = IButtonProps & React.ButtonHTMLAttributes<HTMLElement>;
type IChangeButtonProps = React.ChangeEvent<Element> & React.MouseEventHandler<HTMLElement>
//parital 全部参数可选
export type TButtonProps = Partial<INativeButtonProps & IChangeButtonProps & IAnchorButtonProps>;

/**
* @desc: 按钮组件 
* @author:  zhangyunajiang
* @creatDate 2021-05-01 00:02:49
* @param {Object} {}
* @return  {*}
*/
const Button: React.FC<TButtonProps> = (props) => {
    let { btnType, size, disabled, href, children, target = '', loading, ...restProps } = props;
    //判断这些按钮是否存在 如果不存在 默认复制 primary
    if (!["primary", "primary", "warning", "danger", "default", "info", "success"].includes(btnType)) {
        if (btnType != 'Link') btnType = 'primary';
    }
    //判断className 类名
    const classes = classNames('i-button', {
        [`i-button--${btnType}`]: btnType,
        [`i-button-${size}`]: size,
        [`is-disabled`]: disabled,
        [`is-loading`]: loading
    });
    //判断 btn类型 是Link 并且href有值 才用 a 标签  或者使用原生button标签
    if (btnType === 'Link' && href) {
        return (
            <a href={!disabled ? href : "javascript:;"} className={classes} target={target} {...restProps}>
                {children}
            </a>
        )
    } else {
...
* @desc: 按钮默认导出
* @author:  zhangyunajiang
* @creatDate 2021-05-01 00:03:50
* @return  {*}
*/
Button.defaultProps = {
    disabled: false, //默认不禁用
    loading: false, //默认不加载
    btnType: 'primary', //默认类型
    size: 'small', //默认大小
}
